...
Давайте сделаем что-нибудь веселое, которое<!-- вы будете разбирать пол дня --> наглядно покажет прелесть подпрограмм. Допустим, у нас есть некий персонаж с именем, с силой и золотом (что еще для жизни надо?), и мы хотим его как-нибудь красиво отобразить. Для этого создадим локацию `printCharacter`:
```qsp
$name = $args[0]
strength = args[1]
gold = args[2]
...
Т.е. подпрограммы позволяют повторно использовать код, параметризировать его и в случае чего его можно изменить в угоду новых потребностей, а не бегать по всему проекту и бороться с дубликатами.
...
static void Main(string[] args)
{
    ...
    // здесь `return` нет, потому что пустой тип `void` его не требует. Все остальные типы требуют, о чем компилятор в случае чего сообщит
}
```
...
Что в QSP'е, что в C#, да где угодно должна существовать точка входа, т.е. начальная подпрограмма, с которой начинает исполняться код.
 -->

<!-- TODO: стоит ли тут вводить Method Overloading? -->
<!-- TODO: `out` -->
<!-- TODO: функции высшего порядка, callback'и? -->

<!-- TODO: Default Parameter Value `static void MyMethod(string country = "Norway")` -->
